...
基于 VirtualBox/QEMU 的安卓模拟器（夜神、雷电、逍遥、BlueStacks 4-）安装时会**关闭 Hyper-V 和虚拟机平台**，导致 WSL2 完全无法启动。**卸载模拟器不会自动恢复这些设置！**

**快速修复**（管理员 PowerShell 执行，然后重启电脑）：
...
| 模拟器 | Hyper-V 兼容 |
|---|---|
| 夜神 NoxPlayer | ❌ 会导致 WSL2 崩溃 |
| 雷电 LDPlayer 旧版 | ❌ 会导致 WSL2 崩溃 |
| 逍遥 Xiaoyao | ❌ 会导致 WSL2 崩溃 |
| BlueStacks 4- | ❌ 会导致 WSL2 崩溃 |
| MuMu Pro | ✅ 可共存 |
| 雷电9（Hyper-V 版） | ✅ 可共存 |
| BlueStacks 5+（Hyper-V 模式） | ✅ 可共存 |
...
| `scripts/diagnose.py` | 诊断 WSL2 服务问题：虚拟化状态、.wslconfig、服务状态、CRLF、任务配置 |
...
完整版见 [11 个已知坑点](references/pitfalls.md)，包含症状、根因和解决方案。
...
| 11 | 安卓模拟器关闭虚拟化组件 | 修复 `bcdedit` + 重新启用 VirtualMachinePlatform + WSL 组件 |
